    ...
    const std::vector<Vector3i>& input_faces, Mesh& mesh)
{
    ...
    // 1. Logic to identify boundary edges
    ...
    // 2. Build the degenerate triangle mesh representing boundary edges
    if (b_edges.empty()) {
    ...
    // 3. OPTIMIZATION: Initialize the BVH and the linear vertex cache
    b_data.init(b_mesh);
...
void floatTetWild::AABBWrapper::init_tmp_b_mesh_and_tree(
...
{
    // 1. Build degenerate mesh for temporary boundary
    if (b_edges1.empty() && b_edges2.empty()) {
        ...
        for (int i = 0; i < b_edges2.size(); i++) {
            int facet_idx = (int)b_edges1.size() + i;
            int vert_offset = ((int)b_edges1.size() + i) * 2;
            tmp_b_mesh.facets.set_vertex(facet_idx, 0, vert_offset);
            tmp_b_mesh.facets.set_vertex(facet_idx, 1, vert_offset);
            tmp_b_mesh.facets.set_vertex(facet_idx, 2, vert_offset + 1);
        }
    }

    // 2. OPTIMIZATION: Initialize the BVH and the linear vertex cache
    tmp_data.init(tmp_b_mesh);
